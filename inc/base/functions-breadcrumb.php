		...
		if ( ! is_home() && ! is_front_page() || is_paged() ) {
			...
			if ( is_category() ) {//分类存档
				...
				if ( $thisCat->parent != 0 ) {
					...
					echo $cat_code = str_replace( '<a', '<a class="font-8 font-lg-10" itemprop="breadcrumb"', $cat_code );
				}
				echo $before . '' . single_cat_title( '', false ) . '' . $after;
			} elseif ( is_day() ) {//天存档
				echo '<a class="font-8 font-lg-10" itemprop="breadcrumb" href="' . get_year_link( get_the_time( 'Y' ) ) . '">' . get_the_time( 'Y' ) . '</a> ' . $delimiter . ' ';
				echo '<a class="font-8 font-lg-10" itemprop="breadcrumb"  href="' . get_month_link( get_the_time( 'Y' ), get_the_time( 'm' ) ) . '">' . get_the_time( 'F' ) . '</a> ' . $delimiter . ' ';
				...
				if ( get_post_type() != 'post' ) {//自定义文章类型
					...
					echo '<a class="font-8 font-lg-10" itemprop="breadcrumb" href="' . $homeLink . '/' . $slug['slug'] . '/">' . $post_type->labels->singular_name . '</a> ' . $delimiter . ' ';
				...
				$cat    = $cat[0];
				echo '<a class="font-8 font-lg-10" itemprop="breadcrumb" href="' . get_permalink( $parent ) . '">' . $parent->post_title . '</a> ' . $delimiter . ' ';
				echo $before . get_the_title() . $after;
				...
				while ( $parent_id ) {
					...
					$breadcrumbs[] = '<a class="font-8 font-lg-10" itemprop="breadcrumb" href="' . get_permalink( $page->ID ) . '">' . get_the_title( $page->ID ) . '</a>';
