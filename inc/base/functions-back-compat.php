...
if ( ! function_exists( 'GoSheng_upgrade_notice' ) ) {
	function GoSheng_upgrade_notice() {
		$message = sprintf( __( '狗剩主题需要WordPress 4.7或更高版本。您正在运行%s版，请在更新WordPress后重试。', 'GoSheng-framework' ), $GLOBALS['wp_version'] );
...
if ( ! function_exists( 'GoSheng_customize' ) ) {
	function GoSheng_customize() {
		wp_die(
			sprintf(
				__( '狗剩主题需要WordPress 4.7或更高版本。您正在运行%s版，请在更新WordPress后重试。', 'GoSheng-framework' ),
				$GLOBALS['wp_version']
			),
			'',
			array(
...
if ( ! function_exists( 'GoSheng_preview' ) ) {
	function GoSheng_preview() {
		if ( isset( $_GET['preview'] ) ) {
			wp_die( sprintf( __( '狗剩主题需要WordPress 4.7或更高版本。您正在运行%s版，请在更新WordPress后重试。', 'GoSheng-framework' ), $GLOBALS['wp_version'] ) );
		}
	}
}
